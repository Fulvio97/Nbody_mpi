...
int main(int argc, char *argv[]){
	...
	float *tmp = (float*)malloc((int)((nBodies + p - 1) / p) * sizeof(Particle));
	Particle *tmpPr = (Particle*)tmp;

	/*Tmp variable for the values gathered in every iteration*/
	float *tmpRecv = (float*)malloc(nBodies * sizeof(Particle)); //p * ((nBodies + p - 1) / p)
	...
	for(it = 0; it < iterations; it++){
		...
		if(my_rank == 0){
			...
			relBase = my_rank * ceil(nBodies / p);//((nBodies + p - 1) / p);
			printf("**PROCESS %d** Interval of particles: %d - %d\n", my_rank, relBase, relBase + (int)ceil(nBodies / p)/*((nBodies + p - 1) / p)*/);
			...
			if(my_rank == p - 1 && relBase < nBodies){ //(my_rank * (nBodies + p - 1) / p) + ((nBodies + p - 1) / p)
				while(relBase < nBodies){
					tmpPr[n].x = 0;
					tmpPr[n].y = 0;
					tmpPr[n].z = 0;
					tmpPr[n].vx = 0;
